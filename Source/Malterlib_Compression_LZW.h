	...
	namespace NDataProcessing
	{
		...
		private:
			...
			public:
				...
				{
					int nResult = NMem::fg_MemCmp((const uint8*)m_pBuffer, (const uint8*)_pBuffer->m_pBuffer, fg_Min(m_nBytes, _pBuffer->m_nBytes));
			...
			void *fp_ReAllocPtr(void *_pBlock, aint _Size)
			{
				...
				if(_Size > OldSize)
				{
					...
					NMem::fg_MemClear((uint8*)_pBlock+OldSize, _Size-OldSize);
				}
				...
				while(nDesIndex < _nDstBytes)
				{
					...
					if(nSample >= 256)
						...
						{	// normal case, valid Dictionary Sample
							...
							NMem::fg_MemCopy(pDst+nDesIndex, pNodeSample->m_pBuffer, pNodeSample->m_nBytes);
						...
						{	// out of range Sample
							...
							NMem::fg_MemCopy(pDst+nDesIndex, pDst+nDesIndexSave, nDesIndex-nDesIndexSave);
