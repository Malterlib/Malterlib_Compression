...
namespace NMib
{
	namespace NDataProcessing
	{
		...
		template <typename t_CAllocator = NMib::NMem::CDefaultAllocator>
		class TCCompress_Huffman
		{
			...
			public:
				CHuffmanNode() 
				{	
					NMem::fg_MemClear(this, sizeof(CHuffmanNode));	
				}
				...
				while (iBackNode > 0)
				{
					...
					for (i = iBackNode; i >= 0; i--)
						if(pNodes[i]->m_Frequency >= pNode->m_Frequency)
							break;
					NMem::fg_MemMove(pNodes+i+2, pNodes+i+1, (iBackNode-i)*sizeof(aint));
					pNodes[i+1] = pNode;
					iBackNode++;
				}
		...
		public:
			...
			bool f_CompressHuffman(const void *_pSrc, aint _SrcLen, void *&_pDes, aint &_DesLen)
			{
				...
				NMib::fg_QSort(Nodes, 256, CSort_FrequencyCompare());
				...
				uint8 *pDesPtr = pDes;
				NMem::fg_MemClear(pDesPtr, _DesLen);
				...
				*pDesPtr = nNodes-1;
				...
				{	// the array sorted on frequency so used nodes come first
					NMem::fg_MemCopy(pDesPtr, &Nodes[iCount], NodeSize);
					pDesPtr += NodeSize;
				}
				// sort nodes depending on ascii to can index nodes with its ascii value
				NMib::fg_QSort(Nodes, 256, CSort_ASCIICompare());
			...
			{	
				...
				for(aint iCount = 0; iCount < nNodes; iCount++)
				{
					NMem::fg_MemCopy(&Nodes[iCount], pSrc+iSrc, NodeSize);
					iSrc += NodeSize;
				}
				// construct Huffman tree
